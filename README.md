...
admin.html ページ全体
* admin.html ヘッダー右側各ツールへジャンプするボタンを削除

admin.html スクリプトタブ

admin.html ヒアリングタブ
* プルダウン　手入力用の”その他”は常にメニューの一番下に表示される仕様にしてください（後から編集で項目を追加した時に1,2,その他,3となってしまう）
* ログ作成補助　同じボタンを複数回押下した時、その分だけテキストが挿入されてしまう（同じテキストを2回挿入する必要がない）
* ラジオボタン　解除ができない
* 編集　ボタン押下時、その項目の直下にメニューを表示する
...
* お知らせ　一回目はadmin.html から登録できるが削除できない（admin.html サイドメニュータブ お知らせ のテキストを空にしても、1回目に登録したテキストがindex.html では表示されている状態）
